            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-slate-800 tracking-tight">
                        {t('pipeline_management').split(' ')[0]} <span className="text-yellow-500">{t('management_span')}</span>
                    </h1>
                    <p className="text-slate-400 text-sm mt-1">{t('track_contracts_desc')}</p>
                </div>
                    ...
                    <AnimatePresence mode="popLayout">
                                ...
                                >
                                    <div className="flex flex-col lg:flex-row gap-8">
                                        ...
                                        <div className="flex items-center gap-6 lg:w-1/4">
                                            ...
                                            <div>
                                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">{t('client_node') || 'Client Node'}</p>
                                        ...
                                        <div className="flex-1 space-y-4">
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="p-4 bg-slate-50 rounded-2xl border border-transparent hover:border-slate-200 transition-all">
                                                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{t('service_type')}</p>
                                                ...
                                                <div className={`p-4 rounded-2xl border border-transparent transition-all ${subCfg.color}`}>
                                                    <p className="text-[9px] font-black uppercase tracking-widest opacity-60 mb-1">{t('execution_status')}</p>
                                                    <div className="flex items-center gap-2">
                                                        <subCfg.icon className="text-xs" />
                                                        <span className="text-xs font-black uppercase tracking-widest tracking-tighter">{t(`substatus_${job.subStatus}`) || subCfg.label}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div className="lg:w-1/4 flex flex-col justify-center gap-3">
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    {job.location && (
                                                        <a
                                                            ...
                                                            title={t('check_distance_title') || 'Check Distance on Map'}
                                                        >
                                                            <FaMapMarkerAlt /> {t('view_location')}
                                                        </a>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            {filterStatus === 'active' && (
                                                <div className="space-y-2">
                                                    ...
                                                    {job.subStatus === 'not_started' && (
                                                        ...
                                                        >
                                                            <FaPlay /> {t('start_implementation_btn') || 'Start Implementation'}
                                                        </button>
                                                    )}
                                                    ...
                                                    {job.location && (
                                                        <a
                                                            ...
                                                            title={t('track_location_title') || 'Track Direct Location & Distance'}
                                                        >
                                                            <FaMapMarkerAlt /> {t('track_location')}
                                                        </a>
                                                    )}
                                                    ...
                                                    {job.subStatus === 'in_progress' && (
                                                        ...
                                                        >
                                                            <FaFlagCheckered /> {t('handover_close_btn')}
                                                        </button>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            {filterStatus === 'completed' && (
                                                <div className="flex items-center justify-center gap-2 px-6 py-4 bg-green-50 text-green-600 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-green-100">
                                                    <FaCheckCircle /> {t('successfully_closed_msg')}
                                                </div>
                                            )}
                                        </div>
                                    </div>
                        ...
                        {filteredJobs.length === 0 && (
                            <div className="py-32 text-center bg-white rounded-[2.5rem] border border-dashed border-slate-200">
                                ...
                                <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">{t('pipeline_empty')}</p>
                            </div>
                        )}
                    </AnimatePresence>
                </div>
