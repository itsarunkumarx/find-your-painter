...
const MyBookings = () => {
    ...
    const [callConfig, setCallConfig] = useState({ isOpen: false, type: 'voice', contact: null });
    ...
    const cancelBooking = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const config = { headers: { Authorization: `Bearer ${token}` } };
            await axios.put(`${import.meta.env.VITE_API_URL}/api/bookings/${id}/status`, { status: 'cancelled' }, config);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-slate-800 tracking-tight">
                        My <span className="text-yellow-500">Service History</span>
                    </h1>
                    <p className="text-slate-400 text-sm mt-1">Track your active painter appointments and project progress.</p>
                </div>
                                ...
                                >
                                    <div className="flex flex-col lg:flex-row gap-8">
                                        ...
                                        <div className="flex items-center gap-6 lg:w-1/3">
                                            ...
                                            <div>
                                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Contractor</p>
                                        ...
                                        <div className="flex-1 space-y-4 pt-2">
                                            <div className="flex items-center justify-between">
                                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Project Progress</span>
                                                <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest">{subCfg.label} — {subCfg.progress}%</span>
                                            </div>
                                        ...
                                        <div className="flex items-center gap-3 lg:w-auto shrink-0 justify-end flex-wrap">
                                            {booking.status === 'accepted' && (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button
                                                        onClick={() => setCallConfig({ isOpen: true, type: 'voice', contact: booking.worker.user })}
                                            ...
                                            >
                                                View Mission Intelligence <FaArrowRight className="group-hover:translate-x-1 transition-transform" />
                                            </button>
