    ...
    */
    ...
    const hangUp = useCallback(() => {
        ...
        if (currentCall?.toUserId && socketRef.current) {
            if (currentCall.isCaller && !currentCall.remoteStream) {
                socketRef.current.emit('call_cancel', { toUserId: currentCall.toUserId, reason: 'caller_hangup' });
            } else {
    ...
    const queryHardwarePermissions = useCallback(async () => {
        const result = { mic: 'unknown', camera: 'unknown' };
        if (typeof navigator !== 'undefined' && navigator.permissions) {
            try {
                const micStatus = await navigator.permissions.query({ name: 'microphone' });
                result.mic = micStatus.state;
            } catch (e) {
                result.mic = 'unknown';
            }
            try {
    ...
    const requestHardwareAccess = useCallback(async ({ audio = true, video = true } = {}) => {
        try {
            const constraints = {
                audio: audio ? {
                    echoCancellation: true,
    ...
    const startCall = useCallback(async (contact, callType = 'voice') => {
        ...
        // Parallelize media capture and peer initialization with resilient fallbacks
        const mediaPromise = (async () => {
            ...
            try {
                ...
                // If primary high-spec acquisition fails due to hardware contention, attempt relaxed fallback
                if (['NotReadableError', 'TrackStartError', 'OverconstrainedError'].includes(firstErr.name)) {
                    try {
                        ...
                        // If video fails, try falling back to audio-only
                        if (callType === 'video') {
                            stream = await navigator.mediaDevices.getUserMedia({ audio: true });
                            toast('📹 Video camera was busy. Connected via voice audio.', { icon: 'ℹ️' });
                        } else {
                            throw fallbackErr;
                        }
                    }
                ...
                try {
                    ...
                    if (['NotReadableError', 'TrackStartError', 'OverconstrainedError'].includes(firstErr.name)) {
                        try {
                            ...
                            if (callType === 'video') {
                                stream = await navigator.mediaDevices.getUserMedia({ audio: true });
                                toast('📹 Video camera was busy. Answering via voice audio.', { icon: 'ℹ️' });
                            } else {
                                throw fallbackErr;
                            }
                        }
    ...
    const value = useMemo(() => ({
        ...
        queryHardwarePermissions, requestHardwareAccess
    }), [
