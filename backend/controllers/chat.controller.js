    ...
    try {
        ...
        try {
            ...
            if (booking) {
                ...
                const isClient = booking.user?._id?.toString() === req.user._id.toString() || booking.user?.toString() === req.user._id.toString();
                
                if (isClient) {
                    // Send to Worker's User account
                    if (booking.worker) {
                        recipientId = booking.worker.user?._id || booking.worker.user || booking.worker;
                    }
                } else {
                    // Send to Client (User)
